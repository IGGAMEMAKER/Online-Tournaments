...
function landIsFree(gameID, x, y){
	return games[gameID].map[x][y].owner == OWNER_NONE;
}

function takeLand(gameID, squad,playerID){
	var x=squad.x, y=squad.y;
	games[gameID].map[x][y].owner = playerID;
	games[gameID].map[x][y].squadID = squad.squadID;
}

function freeLand(gameID, x, y){
	games[gameID].map[x][y].owner = OWNER_NONE;
	games[gameID].map[x][y].squadID = OWNER_NONE;
}
...
function CreateArmy(gameID, sizeOfArmy, playerID, Y){
	...
	for (var i=0; i < sizeOfArmy; ++i){
		var x = i;
		var squad = CreateSquad(x, y, i, playerID);
